...
useEffect(() => {
logger.error("Applications page error:", error);
}, [error]);
...
Your campaign applications could not be fetched. Please try again.
</p>
{error.digest && (
<code className="text-xs text-muted font-mono">Ref: {error.digest}</code>
)}
<Button variant="primary" aria-label="Retry loading applications" onClick={() => reset()}>
Try again
</Button>
</div>
);
