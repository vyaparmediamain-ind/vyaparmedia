...
export default function DashboardDisputesError({
error,
reset,
}: Readonly<{ error: Error & { digest?: string }; reset: () => void }>) {
useEffect(() => {
logger.error("Disputes page error:", error);
}, [error]);
...
<h2 className="text-xl font-extrabold">Disputes failed to load</h2>
<p className="text-secondary text-sm text-center max-w-380">
Your dispute history could not be fetched. Please try again.
</p>
{error.digest && (
<code className="text-xs text-muted font-mono">Ref: {error.digest}</code>
)}
<Button variant="primary" aria-label="Retry loading disputes" onClick={() => reset()}>
Try again
</Button>
</div>
);
