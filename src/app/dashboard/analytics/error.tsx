...
useEffect(() => {
logger.error("Dashboard analytics page error:", error);
}, [error]);
...
Your analytics data could not be fetched. Please try again.
</p>
{error.digest && (
<code className="text-xs text-muted font-mono">Ref: {error.digest}</code>
)}
<Button variant="primary" aria-label="Retry loading analytics" onClick={() => reset()}>
Try again
</Button>
</div>
);
