...
<h2 className="text-xl font-extrabold">Settings failed to load</h2>
<p className="text-secondary text-sm text-center max-w-380">
Your account settings could not be fetched. Please try again.
</p>
{error.digest && (
<code className="text-xs text-muted font-mono">Ref: {error.digest}</code>
)}
<Button variant="primary" aria-label="Retry loading settings" onClick={() => reset()}>
Try again
</Button>
</div>
);
